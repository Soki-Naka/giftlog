class StaticPagesController < ApplicationController
  def home
    ...
    # @comments_users_rank = User.joins(posts: :comments).where.not(posts[:user_id].eq(comments[:user_id])).group(comments[:user_id]).select('users.*, COUNT(`comments`.`id`) AS comments_count').order('count(comment_id) DESC').limit(3)
