class StaticPagesController < ApplicationController
  def home
    ...
    # @comments_users_rank = User.joins(posts: :comments).group(comments[:user_id]).having.not(comments[:user_id].eq(posts[:user_id])).select('users.*, COUNT(`comments`.`id`) AS comments_count').order('count(comment_id) desc').limit(3)
